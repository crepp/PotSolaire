...
difference(){
    union(){
        hull(){
            ...
            translate([ep+1.5+ep,0,0]) cylinder(r=1.5+ep, h=tz);
        }
        hull(){
            translate([ep+1.5,ty,0]) cylinder(r=1.5+ep, h=tz);
            translate([ep+1.5+ep,ty,0]) cylinder(r=1.5+ep, h=tz);
        }
    }
    translate([ep+1.5+ep,0,0]) cylinder(r=1.7, h=tz+2);
    translate([ep+1.5+ep,ty,0]) cylinder(r=1.7, h=tz+2);
}
